...
**Backlog confirmado pelo Diogo, ainda não implementado (nesta ordem, salvo indicação em contrário):**

1. **Botão de WhatsApp fixo** no canto inferior direito, nas 3 páginas.
2. **CTA da hero repensado** — o Diogo pediu algo "bem pensado e estruturado", não só os 2 botões atuais (Ver catálogo / Conocer La Norma). Ainda não é uma direção fechada — precisa de uma rodada de grilling própria antes de implementar (o que muda: texto, hierarquia entre os dois CTAs, ou adicionar um terceiro caminho tipo WhatsApp direto?).
3. **Explorar paleta de cor nova, com urgência** — o Diogo apontou que o brass/dourado (o único acento do sistema desde a Rodada 2) **não existe em nenhuma máquina real nem na fábrica** — é uma cor sem base fotográfica real, mesmo problema que já matou o cobre na v1 (seção 6). Ele confirmou que quer testar isso **agora, mas sem perder o que já foi feito** — ou seja: qualquer teste de paleta deve rodar em preview/branch isolado, não sobrescrever o brass já aplicado até uma nova cor ser aprovada de verdade. Próximo passo real: puxar fotos reais do taller/das máquinas (`assets/biblioteca-wp-completa`, fotos que o Diogo mandar) e ver qual cor de acento tem base real, do mesmo jeito que o brass foi escolhido evitando o terracota "tell de IA".
4. **"Formación" no menu do header** — o Diogo confirmou explicitamente que quer o item de menu **agora**, mesmo com o conteúdo da página ainda não confirmado (a regra anterior de "não implementar Formación sem confirmar curso/público/formato" seguia valendo até esta conversa). **Pendência real a resolver antes de implementar:** pra onde esse link aponta — não inventar curso, público ou formato. Opção mais segura: uma página/seção curta e honesta ("em preparação", sem detalhe inventado) até o Diogo trazer a informação real.
5. **Motionographer como referência** — o Diogo pediu pra puxar referência de lá pra uma futura rodada de movimento/animação, cruzando com tudo que já foi decidido no projeto (não é pra implementar cine-scroll agora — isso continua sendo fase própria, seção 6/8).
6. **Deck/material de apresentação para o Daniel** — decisão do Diogo foi ambígua de propósito ("deixar tão bom aqui que quando formos pra lá tá tudo pronto"): a leitura mais segura é **focar 100% no site primeiro**, sem abrir uma frente paralela de slides ainda. Confirmar isso explicitamente no início da próxima sessão antes de tocar em `contexto/`/`apresentacao-daniel/`.
