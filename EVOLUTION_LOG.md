...
## Rodada 8 — 25/09, hero com o texto/tipografia da capa da revista + correções na revista

**Hero:** o Diogo viu a capa da revista de visita ("Hecha a mano. Pensada para durar y repararse.", Fraunces grande sobre a foto do painel) e pediu esse mesmo efeito na hero real do site — foi o que "chamou atenção" nos testes desta rodada. Trocado: `.hero h1` de Archivo bold pra Fraunces peso 500 (mesma família da capa); título e subtítulo **invertidos** — o texto que era lede ("Para negocios que quieren ir un paso más allá") virou subtítulo pequeno, e o texto que era lede de produto ("Fabricadas a mano... pensadas para durar y repararse") virou o H1 grande, com concordância corrigida pro plural (as máquinas): "Hechas a mano. Pensadas para durar y repararse." Nenhuma frase nova — as duas já existiam e eram validadas, só trocaram de papel.

**Revista — dois bugs achados no celular do Diogo:**
1. **Logo quebrada** (ícone de imagem partida no topo das 3 páginas): o publish do artifact simplesmente esqueceu de incluir `materiais/assets/logo-lanorma-cream.png` e `logo-lanorma-brass.png` nos arquivos publicados — não é bug no código, é bug no comando de publish. Corrigido republicando com os dois arquivos.
2. **Texto vazando por trás do rodapé fixo:** `.toolbar` (← Revistas / contador / Imprimir) é position:fixed, mas só as pills individuais tinham fundo — os vãos entre elas deixavam o próximo spread (que fica a só 12px de distância no celular) aparecer por trás, lendo como "texto em cima de texto". Corrigido: no breakpoint ≤700px a barra vira sólida de ponta a ponta (`rgba(20,16,9,.97)` + blur), sem pills flutuantes, e o último spread ganhou `margin-bottom:64px` pra nunca ficar embaixo da barra.

**Aprendizado pra próxima publicação de artifact com pastas fora do diretório da página principal:** o artifact serve a página de entrada e todo `files` como se estivessem na raiz — um `src="../pasta/arquivo.png"` normaliza pra `/pasta/arquivo.png` (o `..` na raiz não sobe, o browser descarta). Publicar o asset com a chave `pasta/arquivo.png` (sem repetir a pasta da página que referencia) resolve; testar localmente servindo a partir da raiz do repo, não de uma subpasta, senão o próprio `http.server` bloqueia o `../` por segurança e mascara esse comportamento do browser.

## Rodada 9 — 25/09, polimento final da hero (posição, contraste, cascata, intro)

**Contexto:** depois da Rodada 8 entrou a cascata de entrada (`8d40450`: eyebrow → h1 → lede → CTA, fade + 18px). Esta rodada foi só acabamento — foto, fonte, texto e composição não foram reabertos. Validado com Playwright em 12 viewports (1440×900/760, 1366×768, 1280×720, 1024×768, 900×1000, 820×1180, 700×900, 390×844, 390×664, 375×667, 360×740), com contraste medido em pixel (pior 5% atrás de cada linha de texto) e a sequência real com a intro. Capturas antes/depois em `review/hero-final/`.

**O que estava errado (e só aparecia com a Fraunces carregada de verdade — ver aprendizado):**
1. **Texto alto demais.** `.hero` herdava `section{padding:112px 0}` + os 96px do `.hero-content` = 208px vazios embaixo. Com o H1 em 3 linhas, o bloco subia até a fileira de xícaras (1440×900), cobria as xícaras (1440×760 — o viewport real de um MacBook Air com barra do navegador), encostava no header (1280×720) e **cobria o display "Lanorma"** no iPhone SE (375×667) e em 360×740 — eyebrow a 1,7–1,9:1 nesses casos. Corrigido: a hero não herda mais o padding de baixo; respiro `clamp(48px,10vh,112px)`, H1 com `min(4.6vw,7.4vh)`, celular mantém os 136px validados, e celular baixo (≤760px de altura — inclui qualquer iPhone com as barras do Safari) ganha espaçamentos menores e CTA empilhado (o link ia pra cima do botão do WhatsApp em 390×664).
2. **Recorte 16:9 em tela quase quadrada** (900×1000) cortava dígitos dos displays laterais ao meio — o mesmo defeito que motivou o `hero-mobile.jpg`. O `<source>` vertical passou de `max-aspect-ratio: 3/4` pra `1/1`.
3. **Eyebrow da intro fantasma por cima da hero** na saída (~720ms): `.is-closing .intro-eyebrow{opacity:0}` perdia pro `animation … forwards`. Corrigido com `animation:none` (o botão Saltar já fazia isso). Afeta as 4 páginas, só pra melhor.
4. **Eyebrow da hero perto da borda iluminada do vapor** (700×900: 4,1:1). Halo escuro só atrás das letras (`text-shadow`) — scrim local, sem escurecer a foto. Testado também `text-wrap:balance` no eyebrow (tira o "·" do fim da linha no celular), **descartado**: empurrava "DE CAFÉ · VALENCIA" pra cima do vapor e derrubava o contraste pra 2,6:1.

**Cascata:** curva e 18px mantidos (mesmo vocabulário da intro; overshoot foi considerado e rejeitado — cara de app, não editorial). Delays de .15/.28/.46/.6 pra .12/.24/.40/.52 (ritmo mais regular, termina em ~1,2s). Folga medida entre o fim da cascata e o início da saída da intro: de 114–162ms pra 213–246ms. Estado invisível movido pro keyframe (`fill-mode:both`) — se a animação não rodar, o texto aparece (falha aberta, verificado com animações canceladas, reduced-motion e sem JS).

**O que já estava bom e não foi tocado:** scrim `.hero::after` (o problema de contraste era posição, não gradiente — com o texto no lugar certo o H1 fica ≥5:1 em tudo), foto/tratamento, 390×844 (idêntico ao validado), 820×1180, larguras 700/900/1024 sem quebra nem overflow horizontal.

**Aprendizado:** o Chromium headless deste ambiente **não carrega as Google Fonts** (o certificado do proxy é recusado) — as primeiras capturas saíram em serif de fallback, com o H1 em 2 linhas, e escondiam o problema 1 inteiro. Pra testar tipografia: `ignoreHTTPSErrors` no contexto, ou baixar o CSS/woff2 com `curl` e servir via `page.route` (mais estável). **Sempre conferir `document.fonts` antes de julgar quebra de linha ou posição.**
